    ...
    <div className="bg-white rounded-lg shadow-lg p-6 max-w-2xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {popularTokens.map((token) => (
            <button
              ...
              disabled={loading}
              className={`p-4 rounded-lg border-2 transition-all duration-200 hover:shadow-md disabled:opacity-50 disabled:cursor-not-allowed ${
      ...
      {selectedToken && (
        <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          ...
          {loading && (
            <p className="text-blue-600 text-sm mt-1">Fetching price data...</p>
          )}
        </div>
      )}
    </div>
